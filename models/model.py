...
# from torch_geometric.nn import ChebConv, GCNConv, SGConv, APPNP, GCN2Conv, JumpingKnowledge, MessagePassing, global_mean_pool # noqa
from torch_geometric.nn import GCNConv, GATConv, GINConv, SGConv, APPNP, GCN2Conv, global_mean_pool
...
class GCNNet(torch.nn.Module):
    def __init__(self, dataset, args):
        ...
        self.conv2 = GCNConv(args.dim, len(dataset.classdict))

        if args.feature_sparse:
            self.x_weight = nn.Parameter(torch.ones_like(dataset.data.x)[0].to(torch.float32))
    ...
    def forward(self, data):
        ...
        if self.args.adj_sparse:
            new_edge_indices = []
            for sample_id in torch.unique(data.batch):
                sample_mask = (data.batch == sample_id)
                
                sample_edge_mask = sample_mask[edge_index[0]] & sample_mask[edge_index[1]]
                sample_edges = edge_index[:, sample_edge_mask]

                x_i = x[sample_edges[0]]
                x_j = x[sample_edges[1]]
                inner_products = (x_i * x_j).sum(dim=1)

                num_edges = sample_edges.size(1)
                num_top_edges = int(num_edges * (1-self.args.curr_density_adj))
                _, top_k_indices = torch.topk(inner_products, num_top_edges)
                
                new_edge_indices.append(sample_edges[:, top_k_indices])

            new_edge_index = torch.cat(new_edge_indices, dim=1)

        x = F.relu(self.conv1(x, new_edge_index))
        x = F.dropout(x, training=self.training)
        x = self.conv2(x, new_edge_index)
...
class GATNet(torch.nn.Module):
    def __init__(self, dataset, args):
        ...
        self.conv2 = GATConv(args.dim, len(dataset.classdict))
...
class GINNet(torch.nn.Module):
    def __init__(self, dataset, args):
        ...
        self.conv2 = GINConv(nn.Linear(args.dim, len(dataset.classdict)))
...
class SGCNet(torch.nn.Module):
    def __init__(self, dataset, args):
        ...
        self.args = args
        self.conv1 = SGConv(dataset.data.x.shape[1], len(dataset.classdict), K=2) # Simplified 2 Layer GCN
...
class APPNPNet(torch.nn.Module):
    def __init__(self, dataset, args):
        ...
        self.args = args
        self.lin1 = Linear(dataset.data.x.shape[1], args.dim)
        self.lin2 = Linear(args.dim, len(dataset.classdict))
        self.prop1 = APPNP(10, 0.1) # 10-hop Approximate Personalized PageRank with 0.1 smooth factor
...
class GCNIINet(torch.nn.Module):
    def __init__(self, dataset, args,):
        ...
        self.lins.append(Linear(dataset.data.x.shape[1], args.dim))
        self.lins.append(Linear(args.dim, len(dataset.classdict)))

        self.convs = torch.nn.ModuleList()
        for layer in range(8): # denote layers of GCNII
            # layer=16, alpha=0.1, theta=0.5
            self.convs.append(GCN2Conv(args.dim, 0.1, 0.5, layer + 1))
