...
def add_sparse_args(parser):
    ...
    parser.add_argument('--prune-rate', type=float, default=0.5, help='The pruning rate / death rate for Zero-Cost Neuroregeneration.')
    parser.add_argument('--pruning-rate', type=float, default=0.5, help='The pruning rate / death rate.')
    ...
    parser.add_argument('--update-frequency', type=int, default=20, metavar='N', help='how many iterations to train between mask update')
    ...
    parser.add_argument('--init-density', type=float, default=1.0, help='The pruning rate / death rate.')
    ...
    parser.add_argument('--final-density_adj', type=float, default=1.0, help='The density of the overall sparse network.')
    parser.add_argument('--init-density_feature', type=float, default=1.0, help='The pruning rate / death rate.')
    parser.add_argument('--final-density_feature', type=float, default=1.0, help='The density of the overall sparse network.')
    parser.add_argument('--init-prune-epoch', type=int, default=0, help='The pruning rate / death rate.')
    parser.add_argument('--final-prune-epoch', type=int, default=200, help='The density of the overall sparse network.')
    parser.add_argument('--rm-first', action='store_true', help='Keep the first layer dense.')
    parser.add_argument('--curr-density_adj', type=float, default=0.0, help='The current pruning rate / death rate.')
...
class Masking(object):
    def __init__(self, optimizer,
        ...
        self.num_trainset_edges = train_loader.dataset.data.edge_index.shape[1]
            ...
            while not is_epsilon_valid:
                ...
                for name, mask in self.masks.items():
                    ...
                    if name in dense_layers:
                        ...
                        raw_probabilities[name] = (np.sum(mask.shape) / np.prod(mask.shape)
                                                  ) ** erk_power_scale
    ...
    def pruning(self, step):
        ...
        if curr_prune_iter >= ini_iter and curr_prune_iter <= final_iter - 1:
            ...
            curr_prune_rate = (1 - self.args.init_density) + (self.args.init_density - self.args.final_density) * (1 - prune_decay)
            # curr_prune_rate_adj = (1 - self.args.init_density_adj) + (self.args.init_density_adj - self.args.final_density_adj) * (1 - prune_decay)
            self.args.curr_density_adj = (1 - self.args.init_density_adj) + (self.args.init_density_adj - self.args.final_density_adj) * (1 - prune_decay)
            curr_prune_rate_feature = (1 - self.args.init_density_feature) + (self.args.init_density_feature - self.args.final_density_feature) * (1 - prune_decay)
            ...
            # print('Total Model parameters:{}, \nGraph Edge Numbers:{}, \nFeature Channels:{}'.format(weight_total_size-1,adj_total_size-1,feature_total_size-1))
            print('Total Model parameters: {} \nGraph Edge Numbers: {} \nFeature Channels: {}'.format(weight_total_size, self.num_trainset_edges, feature_total_size))
            ...
            print('Model Parameters Sparsity after pruning: {:.4f} \nGraph Edge Sparsity after pruning: {:.4f} \nFeature Channels Sparsity after pruning: {:.4f}'.format(
                curr_prune_rate, self.args.curr_density_adj, curr_prune_rate_feature))
    ...
    def pruning_uniform(self, step):
        ...
        if curr_prune_iter >= ini_iter and curr_prune_iter <= final_iter:
            ...
            print('Total Model parameters:{}, \nGraph Edge Numbers:{}, \nFeature Channels:{}'.format(weight_total_size,adj_total_size,feature_total_size))
